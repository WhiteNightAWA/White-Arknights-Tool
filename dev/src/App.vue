  ...
  <v-app>
    <v-main>
      ...
      <div style="width: 100%; height: 100%" v-else>
        <PhonePage style="width: 100%; height: 100%"/>
      </div>
      <v-dialog v-if="isPhone" v-model="open" persistent>
        <v-card >
          <v-card-title>
            {{ $t("phone.wait") }}
          </v-card-title>
          ...
          <v-card-actions class="justify-center">
            <v-btn @click="isPhone = false; $store.state.isPhone = false" color="primary" x-small text>{{ $t("phone.desktop") }}</v-btn>
          </v-card-actions>
          <v-card-actions class="justify-center">
            <v-btn @click="open = false" color="primary" x-small text>{{ $t("phone.phone") }}</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-dialog
