<template>
  <v-row color="primary">
    <v-col class="pa-8" id="stageSelector">
      <v-btn-toggle v-model="stage" style="width: 100%" @change="changeStage()">
                  ...
                  <v-expansion-panels accordion>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <v-expansion-panel-header>
                            {{ zone_table['zones'][zone['zoneId']]["zoneNameFirst"] }} -
                            {{ zone_table['zones'][zone['zoneId']]["zoneNameSecond"] }}
                          </v-expansion-panel-header>
                          <v-expansion-panel-content class="inSideButtonList">
                                          ...
                                          >
                                              ...
                                              <img style="width: 5em; position: absolute; opacity: 0.4;"
                                                   src="@/assets/bossMark_white.png" alt="bossMark"
                                                   v-if="stage['bossMark']">
                                              <img style="width: 5em; position: absolute; opacity: 0.4;"
                                                   src="@/assets/openBook_white.png" alt="openBook"
                                                   v-if="stage['stageId'].startsWith('tr')">
                                              <img style="width: 4em; position: absolute; opacity: 0.4;"
                                                   src="@/assets/story.png" alt="story" v-if="stage['isStoryOnly']">
                                              <img style="width: 5em; position: absolute; opacity: 0.4;"
                                                   src="@/assets/originiums.png" alt="story"
                                                   v-if="stage['stageId'].startsWith('hard_')">
    ...
    <v-col class="pa-8" :cols="4" v-if="info">
      ...
      >
        <div>
          ...
          <img style="width: 3em; top:0;right: 0; position: absolute; opacity: 0.4;" src="@/assets/bossMark_white.png"
               alt="openBook" v-if="stage['bossMark']">
          <img style="width: 3em; top:0;right: 0; position: absolute; opacity: 0.4;" src="@/assets/openBook_white.png"
               alt="openBook" v-if="stage['stageId'].startsWith('tr')">
          <img style="width: 3em; top:0;right: 0; position: absolute; opacity: 0.4;" src="@/assets/story.png"
               alt="story" v-if="stage['isStoryOnly']">
          <img style="width: 3em; top:0;right: 0; position: absolute; opacity: 0.4;" src="@/assets/originiums.png"
               alt="story" v-if="stage['stageId'].startsWith('hard_')">
          <v-divider/>
          <v-simple-table dense v-if="!stageLoading">
          ...
          <v-progress-circular class="ma-5" indeterminate v-else/>
          <v-divider/>
          <v-btn :href="'https://penguin-stats.io/result/stage/'+stage['zoneId']+'/'+stage['stageId']" target="_blank"
                 block large style="height: 3em">
            <v-icon left>mdi-package-variant</v-icon>
            {{ $t("RFP.stage.p-s") }}
            <v-icon right>mdi-open-in-new</v-icon>
          </v-btn>
          <v-btn :href="'https://map.ark-nights.com/map/'+stage['stageId']" target="_blank" block large
                 style="height: 3em">
  ...
  methods: {
    getLevelData: function () {
      if (this.stage !== undefined) {
        let url = `https://raw.githubusercontent.com/Kengxxiao/ArknightsGameData/master/${this.getNowServer['id']}/gamedata/levels/${this.stage["levelId"].toString().toLowerCase()}.json`
        this.stageLoading = true
        setTimeout(async () => {
          await axios.get(url).then(data => {
            this.stageData = data["data"]["options"]
    ...
    changeStage: function () {
      this.getLevelData()
      if (this.stage === undefined) {
        this.$emit("update:selectedStage", "")
      } else {
        this.$emit('update:selectedStage', this.stage['stageId'])
      }
    },
