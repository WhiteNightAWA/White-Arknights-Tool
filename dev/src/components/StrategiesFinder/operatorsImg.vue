<template>
  ...
  >
    ...
    <v-card  style="display:flex; background-color: rgba(0,0,0,0); backdrop-filter: blur(10px); flex-direction: column;" class="align-center justify-center" elevation="0">
      <v-card-title>
        【{{ character["rarity"]+1 }}★】 {{ character["name"] }}
      </v-card-title>
        ...
        <div>
          <v-row>
            ...
            <v-col>
              ...
              <div v-else-if="c.kind === 2">
                <img class="refLogo" style="width: 60px; height: 60px;" :src="require(`@/assets/${character['profession']}.png`)" alt="classLogo">
              </div>
              <div v-else-if="c.kind === 3">
                <img class="refLogo" :src="`https://raw.githubusercontent.com/Aceship/AN-EN-Tags/master/img/ui/subclass/sub_${character['subProfessionId']}_icon.png`" alt="subClassLogo">
              </div>
            </v-col>
          </v-row>
        </div>
